...
try{
    require dirname(__DIR__).'/bootstrap.php';require __DIR__.'/PinterestBrokerService.php';require __DIR__.'/PinterestOAuthEndpoint.php';$root=dirname(__DIR__).'/storage/private/pinterest';
    ...
    $config=json_decode((string)file_get_contents($root.'/config.json'),true,8,JSON_THROW_ON_ERROR);$key=(string)file_get_contents($root.'/key.bin');$broker=new SenseCMS\Website\PinterestBrokerService($root,$key,$config);sodium_memzero($key);unset($config);$body=(string)file_get_contents('php://input',false,null,0,65537);[$status,$headers,$output]=SenseCMS\Website\PinterestOAuthEndpoint::response($_SERVER,$body,$broker);
