"""Read-only production acceptance for the X Publisher workspace."""
...
try:
    ...
    for marker in (b'sensecms-unified-workspace', b'X account', b'0.2.1'):
        assert marker in overview, f'Overview marker missing: {marker.decode()}'
    status, page, _ = request('/social-publishing/x')
    assert status == 200, 'X Publisher page unavailable'
    csrf = re.search(rb'name="csrf" value="([a-f0-9]{64})"', page)[1].decode()
    for marker in (b'social-x-page', b'Add X account', b'data-x-oauth-modal', b'0.1.1'):
        assert marker in page, f'X page marker missing: {marker.decode()}'
    status, body, _ = request('/social-publishing/x/status')
    data = json.loads(body)
    assert status == 200 and data['ok'] and data['data']['connected'] and data['data']['connected_count'] >= 1, 'Connected X account missing'
    status, body, _ = request('/api/social-publishing/editor?post_id=0')
    editor = json.loads(body)
    provider = next(item for item in editor['data']['providers'] if item['slug'] == 'x-publisher')
    assert provider['max_message_length'] == 250 and len(provider['connections']) == data['data']['connected_count'], 'X editor provider contract is invalid'
    status, css, headers = request('/extension-assets/plugin/x-publisher/x.css?v=0.1.1')
    assert status == 200 and b'.x-oauth-modal' in css and 'text/css' in headers.get_content_type(), 'X stylesheet unavailable or invalid'
    status, script, headers = request('/extension-assets/plugin/x-publisher/x.js?v=0.1.1')
    assert status == 200 and b'sensecms.x.oauth' in script and 'javascript' in headers.get_content_type(), 'X script unavailable or invalid'
finally:
    try:
...
print('PASS Production X Publisher UI, connected account and versioned assets are healthy.')
