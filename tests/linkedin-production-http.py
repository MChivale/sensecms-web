"""Production acceptance for LinkedIn OAuth up to, but not including, user consent."""
...
try:
    ...
    status, body, _ = request('/social-publishing/linkedin/connect', {'csrf': csrf}, True, {'Accept': 'application/json', 'X-SenseCMS-Request': '1'})
    assert status == 200
    ...
    assert parts.scheme == 'https' and parts.netloc == 'www.sensecms.com' and parts.path == '/api/social/linkedin/v1/authorize'
    assert re.fullmatch(r'[A-Za-z0-9_-]{43}', query['request'][0])
    status, _, headers = request(parts.path + '?' + parts.query, None, False)
    assert status == 302
    linkedin = urllib.parse.urlparse(headers['Location'])
    params = urllib.parse.parse_qs(linkedin.query)
    private = json.loads((web / 'storage/private/linkedin/config.json').read_text())
    assert linkedin.scheme == 'https' and linkedin.netloc == 'www.linkedin.com' and linkedin.path == '/oauth/v2/authorization'
    assert params['response_type'] == ['code'] and params['client_id'] == [private['client_id']]
    assert params['redirect_uri'] == [base + '/api/social/linkedin/v1/callback']
    assert set(params['scope'][0].split()) == {'openid', 'profile', 'w_member_social'}
    assert params['state'] == query['request']
finally:
    try:
...
print('PASS Production LinkedIn OAuth reaches the exact consent URL without submitting authorization.')
