...
$assert(substr_count($market, 'data-market-item ')===19, 'marketplace presents all CMS products as cards');
$entryMarket=$managed(['title'=>'Marketplace','public_path'=>'/extensions','blocks'=>$marketBlocks]);
$assert(substr_count($entryMarket,'data-market-item ')===19 && str_contains($entryMarket,'marketplace-hero'), 'primary extensions route renders the complete marketplace');
...
$assert(substr_count($entryMarket,'data-market-open href=')===38 && substr_count($entryMarket,'data-market-status hidden')===19, 'all product actions support dialogs with real no-JS detail links');
