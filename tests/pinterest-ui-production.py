...
try:
    status,overview,_=request('/social-publishing');assert status==200 and b'Pinterest board' in overview and b'0.3.0' in overview
    ...
    for marker in (b'social-pinterest-page',b'data-pinterest-oauth-modal',b'social-empty social-connect-empty',b'social-steps',b'social-panel-footer',b'Connect with Pinterest',b'0.1.1'):assert marker in page or (marker==b'Connect with Pinterest' and b'Add Pinterest board' in page),marker
    ...
    status,css,headers=request('/extension-assets/plugin/pinterest-publisher/pinterest.css?v=0.1.1');assert status==200 and b'.pinterest-oauth-modal' in css and 'text/css' in headers.get_content_type()
    status,script,headers=request('/extension-assets/plugin/pinterest-publisher/pinterest.js?v=0.1.1');assert status==200 and b'sensecms.pinterest.oauth' in script and 'javascript' in headers.get_content_type()
