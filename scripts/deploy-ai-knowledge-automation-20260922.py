...
check(not (web/migration).exists() and not (web/'scripts/ai-knowledge-sync.php').exists(),'Automation migration and worker are cleanly pending')
...
try:
    ...
    deploy(migration);applied=int(php(r'''$r=new App\Core\Runtime($argv[1]);$db=App\Core\Runtime::connect($r->read('installed')['database']);echo(new App\Installer\WorkspaceMigration($db,$argv[1]))->apply();''',web,user='sensecms'));check(applied==1,'Knowledge automation migration 041 applied exactly once')
