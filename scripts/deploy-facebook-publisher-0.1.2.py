#!/usr/bin/env python3
"""Pinned production popup update for Facebook Publisher 0.1.2."""
from __future__ import annotations
import datetime, fcntl, json, os, shutil, subprocess, sys
from pathlib import Path

if os.name=="nt" or os.geteuid()!=0 or len(sys.argv)!=2:raise SystemExit("Run as root on Linux: deploy-facebook-publisher-0.1.2.py <private-stage>")
...
check(stage.is_dir() and str(stage).startswith('/root/sense-facebook-popup-'),'Private scoped update stage')
for path in [stage/'.addons/social-publishing/sense-package.json',stage/'.plugins/facebook-publisher/sense-package.json',stage/'.plugins/facebook-publisher/runtime.php',stage/'.plugins/facebook-publisher/views/facebook.php',stage/'.plugins/facebook-publisher/assets/facebook.js',stage/'.plugins/facebook-publisher/assets/facebook.css',stage/'tests/social-package-release.php']:
    check(path.is_file() and not path.is_symlink(),'Candidate file '+path.name)
before=package_state();pkg=before['package'];check(pkg['version']=='0.1.1' and pkg['active'] and pkg['signature_status']=='verified','Installed signed plugin 0.1.1 baseline')
check(publisher.is_file() and web.is_dir(),'Production publisher and installation')
for path in [stage/'.plugins/facebook-publisher/runtime.php',stage/'.plugins/facebook-publisher/views/facebook.php']:run(['php8.5','-l',str(path)])
...
try:
    stamp=datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ');backup=Path('/root/sensecms-backups')/(stamp+'-facebook-publisher-012');backup.mkdir(mode=0o700);receipt['backup']=str(backup)
    ...
    hashes={};public=''
    for kind,slug,version in [('addon','social-publishing','0.1.1'),('plugin','facebook-publisher','0.1.2')]:
    ...
    qa=run(['php8.5',str(testtree/'tests/social-package-release.php'),str(artifacts),str(artifacts/'trust.json')]).decode();check('signed social package checks passed' in qa,'Signed 0.1.2 lifecycle on isolated MariaDB')
    uid=int(run(['id','-u','sensecms']).decode());gid=int(run(['id','-g','sensecms']).decode());install_dir=web/'storage/private'/('facebook-popup-deploy-'+stamp);install_dir.mkdir(mode=0o700);os.chown(install_dir,uid,gid);candidate=install_dir/'plugin-facebook-publisher-0.1.2.zip';shutil.copy2(artifacts/candidate.name,candidate);os.chown(candidate,uid,gid);os.chmod(candidate,0o600)
    result=install(candidate);updated=True;check(result['version']=='0.1.2' and result['updated'],'PackageManager upgraded Facebook Publisher to 0.1.2')
    after=package_state();pkg=after['package'];check(pkg['version']=='0.1.2' and pkg['active'] and pkg['signature_status']=='verified','Signed Facebook Publisher 0.1.2 active')
    check(after['rows']==before['rows'],'Social connection, target and delivery rows preserved')
    view=(web/'plugins/facebook-publisher/views/facebook.php').read_text();script=(web/'plugins/facebook-publisher/assets/facebook.js').read_text();check('data-facebook-oauth-modal' in view and "fetch('/social-publishing/facebook/status'" in script,'Popup OAuth UI and bounded completion polling active')
    ...
    if install_dir is not None and install_dir.is_dir() and install_dir.name.startswith('facebook-popup-deploy-') and install_dir.parent==web/'storage/private':shutil.rmtree(install_dir)
