...
if (PHP_SAPI !== 'cli' || $argc !== 4 || !in_array($argv[2], ['--apply', '--rollback'], true)) {
    exit("Usage: php update-mastodon-marketplace-0.1.1.php <installation> --apply|--rollback <private-backup>\n");
}
umask(0077);
...
$journal = $backup . '/mastodon-marketplace-011-before.json';
$lock = fopen($backup . '/mastodon-marketplace-011.lock', 'c+b');
if (!$lock || !flock($lock, LOCK_EX | LOCK_NB)) throw new RuntimeException('Mastodon marketplace update already running.');

$document = static function (string $path) use ($cms): array {
    $page = $cms->pageAtPath($path);
    $doc = $page ? $cms->builderDocument((int) $page['id']) : null;
    if (!$doc) throw new RuntimeException('Expected Mastodon marketplace page is unavailable: ' . $path);
    return $doc;
};
$save = static function (array $before) use ($cms, $catalog, $owner): void {
    $current = $cms->builderDocument((int) $before['id']);
    if (!$current) throw new RuntimeException('A Mastodon marketplace page disappeared during update.');
    $versions = [];
...
if ($argv[2] === '--rollback') {
    ...
    $restore($state);
    echo "Mastodon marketplace metadata restored to 0.1.0.\n";
    exit;
}
if (file_exists($journal)) throw new RuntimeException('Existing Mastodon marketplace update journal; inspect before rerun.');
$state = ['documents' => [$document('/extensions'), $document('/extensions/catalog/plugin/mastodon-publisher')]];
if (file_put_contents($journal, json_encode($state, JSON_THROW_ON_ERROR | JSON_UNESCAPED_SLASHES), LOCK_EX) === false) throw new RuntimeException('Cannot save Mastodon marketplace recovery journal.');
...
try {
    ...
    if ($listingChanges !== 1 || $detailChanges !== 1) throw new RuntimeException('Unexpected Mastodon marketplace 0.1.0 baseline.');
    $save($listing);
    $save($detail);
    $rendered = json_encode([$document('/extensions')['blocks'], $document('/extensions/catalog/plugin/mastodon-publisher')['blocks']], JSON_THROW_ON_ERROR | JSON_UNESCAPED_SLASHES);
    if (str_contains($rendered, 'Development Preview 0.1.0') || substr_count($rendered, 'Development Preview 0.1.1') < 2) throw new RuntimeException('Mastodon marketplace version verification failed.');
} catch (Throwable $error) {
    $restore($state);
    throw $error;
}

echo "Mastodon marketplace metadata updated to 0.1.1 with a scoped recovery journal.\n";
