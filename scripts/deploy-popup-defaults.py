"""Exact, backed-up popup default deployment. No theme, configuration or database migration."""
import datetime
import fcntl
import hashlib
import json
...
web=Path('/home/sensecms.com/web');qa=Path('/root/sense-popup-nQQo93Ij');baseline=Path('/root/sense-builder-I06xQGCK/.cms/source')
...
core=['app/Http/DashboardController.php']
...
for test,args in [('popup-defaults.php',[]),('builder-defaults.php',[]),('media-limits.php',[]),('workspace-migration.php',[]),('security-regressions.php',['--mysql']),('maintenance-regressions.php',['--mysql']),('themes.php',[])]:
...
candidate={str(p):digest(s) for p,s in sources.items()};acceptance=qa/'popup-preflight.json'
...
backup=Path('/root/sensecms-backups')/(stamp+'-popup-defaults');backup.mkdir(mode=0o700)
...
def invalidate():
    fd,name=tempfile.mkstemp(prefix='popup-probe-',suffix='.php',dir=web/'storage')
    try:
