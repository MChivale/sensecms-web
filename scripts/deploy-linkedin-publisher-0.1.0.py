#!/usr/bin/env python3
"""Pinned production deployment for LinkedIn Publisher 0.1.0 and its OAuth broker."""
...
if os.name == "nt" or os.geteuid() != 0 or len(sys.argv) != 2:
    raise SystemExit("Run as root on Linux: deploy-linkedin-publisher-0.1.0.py <private-stage>")
...
expected = {
    web / "app/Core/LicenseService.php": "07203f83ceca5d99e5115a860ab3fc52473a4e312abf2822932ecf0a7967efe5",
    web / "config/workspace.php": "20d9cb77965dec0b39565e5ffff31c837a2b7c190516730c0745ab7e475c81b4",
    nginx: "cf3ece5f3b024e632a356659ada49ec18ce380b3abe6dd755c68366932df94d5",
}
core_files = ["app/Core/LicenseService.php", "config/workspace.php"]
website_files = ["LinkedInBrokerService.php", "LinkedInOAuthEndpoint.php", "linkedin-oauth.php"]
receipt = {"status": "preflight", "checks": []}
backup = install_dir = None
linkedin_installed = mutated = False
created_website = []
...
def atomic(source, target):
    temp = target.with_name(target.name + ".linkedin-new")
...
def uninstall_linkedin():
    code = r'''$r=$argv[1];require$r.'/bootstrap.php';$x=new App\Core\Runtime($r);$db=App\Core\Runtime::connect($x->read('installed')['database']);$m=new App\Core\PackageManager($db,$r,'1.0.0');$owner=(int)$db->query("SELECT ur.user_id FROM user_roles ur JOIN roles x ON x.id=ur.role_id WHERE x.slug='owner' ORDER BY ur.user_id LIMIT 1")->fetchColumn();if($m->package('plugin','linkedin-publisher'))$m->uninstall('plugin','linkedin-publisher',$owner);'''
...
required = [
    ...
    *(stage / ".cms/source" / name for name in core_files),
    ...
    stage / ".plugins/linkedin-publisher/sense-package.json",
    stage / ".plugins/linkedin-publisher/plugin.json",
    stage / "deploy/nginx/sensecms.com.conf",
    stage / "tests/linkedin-broker.php",
    stage / "tests/linkedin-publisher.php",
    stage / "tests/linkedin-social-release.php",
    stage / "tests/linkedin-ui-production.py",
]
check(stage.is_dir() and str(stage).startswith("/root/sense-linkedin-publisher-"), "Private scoped deployment stage")
check(all(path.is_file() and not path.is_symlink() for path in required), "Complete LinkedIn candidate payload")
...
check(all(not (web / "website" / name).exists() for name in website_files), "LinkedIn broker files absent before first deployment")
before = state()
...
check(before["packages"]["plugin:linkedin-publisher"] is None, "LinkedIn Publisher absent before first installation")
for path in [*stage.glob(".src/*.php"), *stage.glob(".plugins/linkedin-publisher/**/*.php"), stage / "tests/linkedin-broker.php", stage / "tests/linkedin-publisher.php", stage / "tests/linkedin-social-release.php", stage / "scripts/provision-linkedin-broker.php"]:
    run(["php8.5", "-l", str(path)])
run(["python3", "-m", "py_compile", str(stage / "tests/linkedin-ui-production.py"), str(stage / "tests/linkedin-production-http.py"), str(stage / "scripts/deploy-linkedin-publisher-0.1.0.py")])
...
try:
    ...
    releases = [("addon", "social-publishing", "0.2.1"), ("plugin", "linkedin-publisher", "0.1.0")]
    hashes = {}
    public = ""
    ...
    (artifacts / "linkedin-social-release.json").write_text(json.dumps(hashes, separators=(",", ":")))
    (artifacts / "trust.json").write_text(json.dumps({"sensecms-release": public}, separators=(",", ":")))
    os.chmod(artifacts / "linkedin-social-release.json", 0o600)
    ...
    for name in ["linkedin-broker.php", "linkedin-publisher.php", "linkedin-social-release.php"]:
        shutil.copy2(stage / "tests" / name, testtree / "tests" / name)
    check("21 LinkedIn broker checks passed" in run(["php8.5", str(testtree / "tests/linkedin-broker.php")]).decode(), "Isolated LinkedIn OAuth broker protocol")
    check("16 LinkedIn Publisher protocol checks passed" in run(["php8.5", str(testtree / "tests/linkedin-publisher.php")]).decode(), "Isolated LinkedIn publishing protocol")
    check("signed LinkedIn social package checks passed" in run(["php8.5", str(testtree / "tests/linkedin-social-release.php"), str(artifacts), str(artifacts / "trust.json")]).decode(), "Exact signed LinkedIn package lifecycle on isolated MariaDB")

    stamp = datetime.datetime.now(datetime.timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    backup = Path("/root/sensecms-backups") / (stamp + "-linkedin-publisher-010")
    ...
    install_dir = web / "storage/private" / ("linkedin-publisher-deploy-" + stamp)
    install_dir.mkdir(mode=0o700)
    os.chown(install_dir, uid, gid)
    candidate = install_dir / "plugin-linkedin-publisher-0.1.0.zip"
    shutil.copy2(artifacts / candidate.name, candidate)
    os.chown(candidate, uid, gid)
    os.chmod(candidate, 0o600)
    result = install(candidate)
    linkedin_installed = True
    check(result["version"] == "0.1.0", "PackageManager installed LinkedIn Publisher 0.1.0")

    after = state()
    linkedin_after = after["packages"]["plugin:linkedin-publisher"]
    check(linkedin_after and linkedin_after["version"] == "0.1.0" and linkedin_after["active"] and linkedin_after["signature_status"] == "verified", "Signed LinkedIn Publisher 0.1.0 active")
    ...
    for source, deployed in [
        ...
        *((stage / ".src" / name, web / "website" / name) for name in website_files),
        (stage / ".plugins/linkedin-publisher/runtime.php", web / "plugins/linkedin-publisher/runtime.php"),
        (stage / ".plugins/linkedin-publisher/src/provider.php", web / "plugins/linkedin-publisher/src/provider.php"),
    ]:
    ...
    run(["python3", str(stage / "tests/linkedin-ui-production.py")])
    check(True, "Authenticated production LinkedIn workspace and assets")
    request = urllib.request.Request("https://www.sensecms.com/api/social/linkedin/v1/start", data=b"{}", headers={"Content-Type":"application/json"}, method="POST")
    try:
        urllib.request.urlopen(request, timeout=30)
        api_status = 200
    except urllib.error.HTTPError as error:
        api_status = error.code
    check(api_status == 503, "LinkedIn broker route is live and fails closed before private provisioning")
    run(["systemctl", "is-active", "nginx", "php8.5-fpm", "mariadb", "cron"])
    run(["nginx", "-t"])
    receipt["status"] = "deployed-awaiting-linkedin-secret"
    shutil.rmtree(install_dir)
    install_dir = None
except BaseException:
    receipt["status"] = "rolling-back" if mutated else "preflight-failed"
    if mutated and backup is not None:
        if linkedin_installed:
            try:
                uninstall_linkedin()
            except BaseException:
                pass
    ...
    if install_dir is not None and install_dir.is_dir() and install_dir.name.startswith("linkedin-publisher-deploy-") and install_dir.parent == web / "storage/private":
        shutil.rmtree(install_dir)
