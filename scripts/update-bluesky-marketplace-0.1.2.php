...
if (PHP_SAPI !== 'cli' || $argc !== 4 || !in_array($argv[2], ['--apply', '--rollback'], true)) {
    exit("Usage: php update-bluesky-marketplace-0.1.2.php <installation> --apply|--rollback <private-backup>\n");
}
umask(0077);
...
$journal = $backup . '/bluesky-marketplace-012-before.json';
$lock = fopen($backup . '/bluesky-marketplace-012.lock', 'c+b');
if (!$lock || !flock($lock, LOCK_EX | LOCK_NB)) throw new RuntimeException('Bluesky marketplace update already running.');

$document = static function (string $path) use ($cms): array {
    $page = $cms->pageAtPath($path);
    $doc = $page ? $cms->builderDocument((int) $page['id']) : null;
    if (!$doc) throw new RuntimeException('Expected Bluesky marketplace page is unavailable: ' . $path);
    return $doc;
};
$save = static function (array $before) use ($cms, $catalog, $owner): void {
    $current = $cms->builderDocument((int) $before['id']);
    if (!$current) throw new RuntimeException('A Bluesky marketplace page disappeared during update.');
    $versions = [];
...
if ($argv[2] === '--rollback') {
    ...
    $restore($state);
    echo "Bluesky marketplace metadata restored to 0.1.1.\n";
    exit;
}
if (file_exists($journal)) throw new RuntimeException('Existing Bluesky marketplace update journal; inspect before rerun.');
$state = ['documents' => [$document('/extensions'), $document('/extensions/catalog/plugin/bluesky-publisher')]];
if (file_put_contents($journal, json_encode($state, JSON_THROW_ON_ERROR | JSON_UNESCAPED_SLASHES), LOCK_EX) === false) throw new RuntimeException('Cannot save Bluesky marketplace recovery journal.');

try {
    $listing = $state['documents'][0];
    $detail = $state['documents'][1];
    $listingChanges = $detailChanges = 0;
    $replace($listing['blocks'], 'Signed Development Preview 0.1.1 available', 'Signed Development Preview 0.1.2 available', $listingChanges);
    $replace($detail['blocks'], 'Development Preview 0.1.1', 'Development Preview 0.1.2', $detailChanges);
    if ($listingChanges !== 1 || $detailChanges !== 1) throw new RuntimeException('Unexpected Bluesky marketplace 0.1.1 baseline.');
    $save($listing);
    $save($detail);
    $rendered = json_encode([$document('/extensions')['blocks'], $document('/extensions/catalog/plugin/bluesky-publisher')['blocks']], JSON_THROW_ON_ERROR | JSON_UNESCAPED_SLASHES);
    if (str_contains($rendered, 'Development Preview 0.1.1') || substr_count($rendered, 'Development Preview 0.1.2') < 2) throw new RuntimeException('Bluesky marketplace version verification failed.');
} catch (Throwable $error) {
    $restore($state);
    throw $error;
}

echo "Bluesky marketplace metadata updated to 0.1.2 with a scoped recovery journal.\n";
