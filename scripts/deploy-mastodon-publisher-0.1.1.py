#!/usr/bin/env python3
"""Build, verify, upgrade and distribute Mastodon Publisher 0.1.1."""
...
if os.name == "nt" or os.geteuid() != 0 or len(sys.argv) != 2:
    raise SystemExit("Run as root on Linux: deploy-mastodon-publisher-0.1.1.py <private-stage>")
...
expected_archive = "a8a272a327ff3034579c3013fa7f86bbf151778af220a00711e8ae3fbb0286e1"
expected_plugin_json = "abfc48a1c1658ae97f8046f39858f400058b010d6067a6850be4d98d85d378ab"
expected_view = "dc3afaf7521a00c5be760af806554500be0b6e4bdacabc82fc288c301a02f2d3"
...
def atomic_bytes(destination, payload):
    temporary = destination.with_name(destination.name + ".mastodon-011-new")
...
def rollback_package():
    code = r'''$r=$argv[1];require$r.'/bootstrap.php';$x=new App\Core\Runtime($r);$db=App\Core\Runtime::connect($x->read('installed')['database']);$m=new App\Core\PackageManager($db,$r,'1.0.0');$owner=(int)$db->query("SELECT ur.user_id FROM user_roles ur JOIN roles x ON x.id=ur.role_id WHERE x.slug='owner' ORDER BY ur.user_id LIMIT 1")->fetchColumn();echo json_encode($m->rollback('plugin','mastodon-publisher',$owner),JSON_THROW_ON_ERROR);'''
    return json.loads(php(code, web, user="sensecms"))


required = [
    stage / ".addons/social-publishing/sense-package.json",
    stage / ".plugins/mastodon-publisher/sense-package.json",
    stage / ".plugins/mastodon-publisher/plugin.json",
    stage / ".plugins/mastodon-publisher/runtime.php",
    stage / ".plugins/mastodon-publisher/views/mastodon.php",
    stage / ".plugins/mastodon-publisher/src/MastodonClient.php",
    stage / "tests/mastodon-publisher.php",
    stage / "tests/mastodon-social-release.php",
    stage / "tests/mastodon-ui-production.py",
]
check(stage.is_dir() and str(stage).startswith("/root/sense-mastodon-publisher-fix-"), "Private scoped Mastodon deployment stage")
check(all(path.is_file() and not path.is_symlink() for path in required), "Complete Mastodon 0.1.1 candidate")
...
check(sha(web / "plugins/mastodon-publisher/plugin.json") == expected_plugin_json and sha(web / "plugins/mastodon-publisher/views/mastodon.php") == expected_view, "Pinned Mastodon 0.1.0 runtime baseline")
before = state()
check(before["plugin"] == {"version": "0.1.0", "active": 1, "signature_status": "verified"}, "Signed Mastodon Publisher 0.1.0 baseline")
config = json.loads(distribution.read_text())
offer = config.get("products", {}).get(identity, {})
check(offer.get("version") == "0.1.0" and offer.get("sha256") == expected_archive and offer.get("pricing") == "paid" and offer.get("enabled") is True, "Paid Mastodon 0.1.0 distribution baseline")
...
run(["python3", "-m", "py_compile", str(stage / "scripts/deploy-mastodon-publisher-0.1.1.py"), str(stage / "tests/mastodon-ui-production.py")])
...
try:
    ...
    hashes = {}
    public = ""
    for kind, slug, version in [("addon", "social-publishing", "0.2.1"), ("plugin", "mastodon-publisher", "0.1.1")]:
    ...
    (artifacts / "mastodon-social-release.json").write_text(json.dumps(hashes, separators=(",", ":")))
    (artifacts / "trust.json").write_text(json.dumps({"sensecms-release": public}, separators=(",", ":")))
    os.chmod(artifacts / "mastodon-social-release.json", 0o600)
    ...
    backup = Path("/root/sensecms-backups") / (stamp + "-mastodon-publisher-011")
    backup.mkdir(mode=0o700)
    receipt["backup"] = str(backup)
    shutil.copy2(distribution, backup / "distribution-before.json")
    shutil.copytree(web / "plugins/mastodon-publisher", backup / "plugin-0.1.0")
    shutil.copy2(artifacts / "plugin-mastodon-publisher-0.1.1.zip", backup / "plugin-mastodon-publisher-0.1.1.zip")
    ...
    install_dir = web / "storage/private" / ("mastodon-publisher-deploy-" + stamp)
    install_dir.mkdir(mode=0o700)
    os.chown(install_dir, uid, gid)
    candidate = install_dir / "plugin-mastodon-publisher-0.1.1.zip"
    shutil.copy2(artifacts / candidate.name, candidate)
    os.chown(candidate, uid, gid)
    os.chmod(candidate, 0o600)
    result = install(candidate)
    installed = True
    check(result.get("version") == "0.1.1" and result.get("updated") is True, "PackageManager upgraded Mastodon Publisher to 0.1.1")

    target = releases / candidate.name
    check(not target.exists(), "Immutable Mastodon 0.1.1 distribution target is new")
    ...
    current_offer = current["products"][identity]
    current_offer.update({"version": "0.1.1", "sha256": hashlib.sha256(payload).hexdigest(), "bytes": len(payload), "file": target.name})
    atomic_bytes(distribution, (json.dumps(current, separators=(",", ":")) + "\n").encode())
    distribution_changed = True

    after = state()
    check(after["plugin"] == {"version": "0.1.1", "active": 1, "signature_status": "verified"}, "Signed Mastodon Publisher 0.1.1 active")
    ...
    for source in stage.glob(".plugins/mastodon-publisher/**/*"):
        if source.is_file() and source.name != "sense-package.json":
            check(sha(source) == sha(web / "plugins/mastodon-publisher" / source.relative_to(stage / ".plugins/mastodon-publisher")), "Deployed checksum " + source.name)
    live = json.loads(distribution.read_text())["products"][identity]
    check(live["version"] == "0.1.1" and live["sha256"] == sha(target) and live["pricing"] == "paid", "Paid Mastodon 0.1.1 distribution offer")
    if after["due"] == 0:
    ...
    if target is not None and target.is_file() and target.parent == releases and target.name == "plugin-mastodon-publisher-0.1.1.zip":
        target.unlink()
    if installed:
        rollback_package()
    run(["systemctl", "reload", "php8.5-fpm"])
    ...
    if install_dir is not None and install_dir.is_dir() and install_dir.name.startswith("mastodon-publisher-deploy-") and install_dir.parent == web / "storage/private":
