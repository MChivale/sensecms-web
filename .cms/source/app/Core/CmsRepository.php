...
final class CmsRepository
{
    ...
    public function savePost(array $input, array $translations, int $userId): int
    {
        ...
        $workflow=in_array($input['status'],['published','scheduled'],true)?'approved':'draft';$this->db->beginTransaction();try{$post=['facility_id'=>$facilityId,'category_id'=>$input['category_id']?:null,'featured_media_id'=>$featured,'audio_media_id'=>$audio,'video_media_id'=>$video,'status'=>$input['status'],'ai_knowledge_enabled'=>!empty($input['ai_knowledge_enabled'])?1:0,'published_at'=>$publishedAt,'assigned_user_id'=>(int)($input['assigned_user_id']??0)?:null,'editorial_note'=>mb_substr(trim((string)($input['editorial_note']??'')),0,1000)?:null,'workflow_state'=>$workflow];if($id){$statement=$this->db->prepare('UPDATE posts SET facility_id=:facility_id,category_id=:category_id,featured_media_id=:featured_media_id,audio_media_id=:audio_media_id,video_media_id=:video_media_id,status=:status,ai_knowledge_enabled=:ai_knowledge_enabled,published_at=:published_at,assigned_user_id=:assigned_user_id,editorial_note=:editorial_note,workflow_state=:workflow_state,updated_at=NOW() WHERE id=:id');$statement->execute($post+['id'=>$id]);if(!$statement->rowCount()&&!$this->postAdmin($id))throw new \RuntimeException('The post was not found.');}else{$this->db->prepare('INSERT INTO posts (author_id,facility_id,owner_user_id,assigned_user_id,workflow_state,editorial_note,category_id,featured_media_id,audio_media_id,video_media_id,status,ai_knowledge_enabled,published_at,created_at,updated_at) VALUES (:author_id,:facility_id,:owner_user_id,:assigned_user_id,:workflow_state,:editorial_note,:category_id,:featured_media_id,:audio_media_id,:video_media_id,:status,:ai_knowledge_enabled,:published_at,NOW(),NOW())')->execute($post+['author_id'=>$input['author_id'],'owner_user_id'=>$userId]);$id=(int)$this->db->lastInsertId();}$upsert=$this->db->prepare('INSERT INTO post_translations (post_id,facility_id,locale,title,slug,excerpt,content,seo_title,seo_description) VALUES (?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE facility_id=VALUES(facility_id),title=VALUES(title),slug=VALUES(slug),excerpt=VALUES(excerpt),content=VALUES(content),seo_title=VALUES(seo_title),seo_description=VALUES(seo_description)');foreach($translations as$locale=>$value)$upsert->execute([$id,$facilityId,$locale,$value['title'],$value['slug'],$value['excerpt'],$value['content'],$value['seo_title'],$value['seo_description']]);$this->savePostTags($id,(array)($input['tags']??[]));$this->audit($userId,'post.saved','post',$id,['status'=>$input['status'],'workflow_state'=>$workflow,'facility_id'=>$facilityId,'featured_media_id'=>$featured,'audio_media_id'=>$audio,'video_media_id'=>$video,'ai_knowledge_enabled'=>(bool)$post['ai_knowledge_enabled']]);$this->db->commit();$this->events->dispatch('post.updated',['post_id'=>$id]);return$id;}catch(\Throwable$error){$this->db->rollBack();throw$error;}
    }
