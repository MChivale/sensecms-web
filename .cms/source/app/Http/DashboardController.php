...
final class DashboardController extends Controller
{
    ...
    public function pageBuilder(): never
    {
        ...
        $document['media']=$this->media->qualityMetadata(PageBuilder::mediaPaths($document['blocks'],$catalog),[(int)$document['facility_id']]);
        $languages=$this->cms->languages();$defaultLocale=$this->cms->defaultLocale('en');
        $facilityId=(int)$document['facility_id'];$canEdit=$this->access->allows('content.pages.edit',$facilityId);$canReview=$this->access->allows('content.pages.review',$facilityId);$collaborators=$this->access->builderCollaborators($facilityId);
        $aiAllowed=$canEdit&&$this->access->allows('content.pages.ai',$facilityId);$aiProviders=[];if($aiAllowed)foreach($this->ai->enabledProviders('builder')as$provider)$aiProviders[]=['slug'=>(string)$provider['slug'],'name'=>(string)$provider['name'],'driver'=>(string)$provider['driver'],'model'=>(string)$provider['default_model'],'verified'=>(bool)($provider['verified_at']??false)];
        $this->render('Page Builder', 'page-builder', ['pages'=>$pages,'builderDocument'=>$document,'builderDraft'=>$canEdit?$this->cms->builderDraft($selected,$this->auth->id()??0):null,'builderClipboard'=>$canEdit?$this->cms->builderClipboard($this->auth->id()??0):null,'builderCatalog'=>$catalog,'builderPatterns'=>PageBuilder::patterns($catalog),'builderLayouts'=>PageBuilder::layoutPresets(),'builderAppearance'=>PageBuilder::appearanceOptions(),'builderCollections'=>$this->cms->builderCollectionOptions($facilityId,$defaultLocale,$defaultLocale,$selected),'builderQuality'=>PageBuilder::qualityReport($document['blocks'],$catalog,$languages,$document),'builderGlobals'=>$this->cms->globalSections(),'builderRevisions'=>$this->cms->builderRevisions($selected),'builderCollaboration'=>$this->cms->builderCollaboration($selected,$this->auth->id()??0),'builderCollaborators'=>$collaborators,'builderAiProviders'=>$aiProviders,'builderAiActions'=>PageBuilder::aiActions($this->plugins->all(),$this->cms->activePluginSlugs(),$this->addons->all(),array_keys(array_filter($this->extensionStates()))),'builderPermissions'=>['edit'=>$canEdit,'review'=>$canReview,'ai'=>$aiAllowed],'languages'=>$languages,'builderTheme'=>$theme]);
    }
    public function savePageBuilder(int $pageId): never
    {
        ...
        $this->editableBuilderDocument($pageId);
    ...
    public function savePageBuilderDraft(int $pageId): never
    {
        $this->guard();$payload=json_decode((string)file_get_contents('php://input'),true);
        if(!is_array($payload))$this->result(false,'The Page Builder recovery payload is invalid.',null,400);
        if(!$this->auth->verifyCsrf($payload['csrf']??null))$this->result(false,'Your session token is invalid. Refresh and try again.',null,419);
        $this->editableBuilderDocument($pageId);$themeSlug=(string)$this->cms->setting('active_theme','sensecms');$catalog=$this->builderCatalog($this->themes->find($themeSlug));$locales=array_column($this->cms->languages(),'locale');
        try{$blocks=PageBuilder::sanitizeBlocks((array)($payload['blocks']??[]),$catalog,$locales,true);$draft=$this->cms->saveBuilderDraft($pageId,$this->auth->id()??0,max(0,(int)($payload['version']??0)),$blocks);$this->result(true,'Recovery draft saved.',null,200,$draft);}catch(\DomainException$error){$this->result(false,$error->getMessage(),null,409);}catch(\RuntimeException$error){$this->result(false,$error->getMessage(),null,422);}
    }
    public function inspectPageBuilder(int$pageId):never
    {
        $this->guard();$payload=json_decode((string)file_get_contents('php://input'),true);
        if(!is_array($payload))$this->result(false,'The quality inspection payload is invalid.',null,400);
        if(!$this->auth->verifyCsrf($payload['csrf']??null))$this->result(false,'Your session token is invalid. Refresh and try again.',null,419);
        $document=$this->editableBuilderDocument($pageId);$themeSlug=(string)$this->cms->setting('active_theme','sensecms');$catalog=$this->builderCatalog($this->themes->find($themeSlug));$languages=$this->cms->languages();$locales=array_column($languages,'locale');
        try{$blocks=PageBuilder::sanitizeBlocks((array)($payload['blocks']??[]),$catalog,$locales,true);$document['media']=$this->media->qualityMetadata(PageBuilder::mediaPaths($blocks,$catalog),[(int)$document['facility_id']]);$this->result(true,'Quality inspection completed.',null,200,PageBuilder::qualityReport($blocks,$catalog,$languages,$document));}catch(\RuntimeException$error){$this->result(false,$error->getMessage(),null,422);}
    }
    public function proposePageBuilderAi(int$pageId):never
    {
        $this->guard();$payload=$this->jsonPayload();$this->builderCsrf($payload);$document=$this->cms->builderDocument($pageId);if(!$document)$this->result(false,'The selected page no longer exists.',null,404);$facility=(int)$document['facility_id'];$this->access->assert('content.pages.edit',$facility);$this->access->assert('content.pages.ai',$facility);if(!$this->aiContent)$this->result(false,'The Core AI service is unavailable.',null,503);
        $themeSlug=(string)$this->cms->setting('active_theme','sensecms');$catalog=$this->builderCatalog($this->themes->find($themeSlug));$languages=$this->cms->languages();$locales=array_column($languages,'locale');try{$blocks=PageBuilder::sanitizeBlocks((array)($payload['blocks']??[]),$catalog,$locales,true);$document['media']=$this->media->qualityMetadata(PageBuilder::mediaPaths($blocks,$catalog),[$facility]);$quality=PageBuilder::qualityReport($blocks,$catalog,$languages,$document);$actions=PageBuilder::aiActions($this->plugins->all(),$this->cms->activePluginSlugs(),$this->addons->all(),array_keys(array_filter($this->extensionStates())));$proposal=$this->aiContent->propose($pageId,$facility,$this->auth->id()??0,$payload,$blocks,$catalog,$languages,$actions,$quality);$this->result(true,'AI proposal ready for review.',null,200,$proposal);}catch(\RuntimeException$error){$status=$error->getCode()===429?429:422;$this->result(false,$error->getMessage(),null,$status);}catch(\Throwable$error){error_log('Page Builder AI proposal failed: '.$error->getMessage());$this->result(false,'The AI proposal could not be completed. Try another configured provider.',null,503);}
    }
    public function discardPageBuilderDraft(int $pageId): never
    {
        $this->guard();$payload=json_decode((string)file_get_contents('php://input'),true);
        if(!is_array($payload)||!$this->auth->verifyCsrf($payload['csrf']??null))$this->result(false,'Your session token is invalid. Refresh and try again.',null,419);
        $this->editableBuilderDocument($pageId);$this->cms->discardBuilderDraft($pageId,$this->auth->id()??0);$this->result(true,'Recovery draft discarded.');
    }
    public function copyPageBuilderSections(int $pageId):never
    {
        $this->guard();$payload=json_decode((string)file_get_contents('php://input'),true);
        if(!is_array($payload))$this->result(false,'The section clipboard payload is invalid.',null,400);
        if(!$this->auth->verifyCsrf($payload['csrf']??null))$this->result(false,'Your session token is invalid. Refresh and try again.',null,419);
        $this->editableBuilderDocument($pageId);$themeSlug=(string)$this->cms->setting('active_theme','sensecms');$catalog=$this->builderCatalog($this->themes->find($themeSlug));$locales=array_column($this->cms->languages(),'locale');
        try{$raw=(array)($payload['blocks']??[]);if(!$raw||count($raw)>20)throw new \RuntimeException('Choose between 1 and 20 sections to copy.');$blocks=PageBuilder::sanitizeBlocks($raw,$catalog,$locales,true);foreach($blocks as&$block){$block['global_section_id']=null;$block['global_name']='';$block['global_version']=0;}unset($block);$clipboard=$this->cms->saveBuilderClipboard($this->auth->id()??0,$pageId,$blocks);$count=count($blocks);$this->result(true,$count.' '.($count===1?'section':'sections').' copied to your private clipboard.',null,200,$clipboard);}catch(\RuntimeException$error){$this->result(false,$error->getMessage(),null,422);}
    }
    public function clearPageBuilderClipboard():never
    {
        $this->guard();$payload=json_decode((string)file_get_contents('php://input'),true);
        if(!is_array($payload)||!$this->auth->verifyCsrf($payload['csrf']??null))$this->result(false,'Your session token is invalid. Refresh and try again.',null,419);
        $this->cms->clearBuilderClipboard($this->auth->id()??0);$this->result(true,'Section clipboard cleared.');
    }
    ...
    public function pageBuilderCollaboration(int$pageId):never
    {
        $this->guard();$this->collaborativeBuilderDocument($pageId);$this->result(true,'Page collaboration updated.',null,200,$this->cms->builderCollaboration($pageId,$this->auth->id()??0));
    }

    public function savePageBuilderSectionWorkflow(int$pageId,string$uid):never
    {
        $this->guard();$payload=$this->jsonPayload();$this->builderCsrf($payload);$document=$this->collaborativeBuilderDocument($pageId);$status=(string)($payload['status']??'draft');if($status==='approved'&&!$this->access->allows('content.pages.review',(int)$document['facility_id']))$this->result(false,'Only a reviewer can approve a section.',null,403);$collaborators=$this->access->builderCollaborators((int)$document['facility_id']);$assigned=max(0,(int)($payload['assigned_user_id']??0))?:null;try{$data=$this->cms->saveBuilderSectionWorkflow($pageId,$uid,$status,$assigned,$this->auth->id()??0,array_column($collaborators,'id'));$this->result(true,'Section workflow updated.',null,200,$data);}catch(\RuntimeException$error){$this->result(false,$error->getMessage(),null,422);}
    }

    public function addPageBuilderSectionComment(int$pageId,string$uid):never
    {
        $this->guard();$payload=$this->jsonPayload();$this->builderCsrf($payload);$this->collaborativeBuilderDocument($pageId);try{$data=$this->cms->addBuilderSectionComment($pageId,$uid,(string)($payload['body']??''),$this->auth->id()??0);$this->result(true,'Comment posted.',null,200,$data);}catch(\RuntimeException$error){$this->result(false,$error->getMessage(),null,422);}
    }

    public function resolvePageBuilderComment(int$pageId,int$commentId):never
    {
        $this->guard();$payload=$this->jsonPayload();$this->builderCsrf($payload);$this->collaborativeBuilderDocument($pageId);try{$resolved=!array_key_exists('resolved',$payload)||(bool)$payload['resolved'];$data=$this->cms->resolveBuilderSectionComment($pageId,$commentId,$resolved,$this->auth->id()??0);$this->result(true,$resolved?'Comment resolved.':'Comment reopened.',null,200,$data);}catch(\RuntimeException$error){$this->result(false,$error->getMessage(),null,422);}
    }

    public function lockPageBuilderSection(int$pageId,string$uid):never
    {
        $this->guard();$payload=$this->jsonPayload();$this->builderCsrf($payload);$this->access->assert('content.pages.edit',(int)$this->collaborativeBuilderDocument($pageId)['facility_id']);try{$data=$this->cms->acquireBuilderSectionLock($pageId,$uid,$this->auth->id()??0);$this->result(true,'Section reserved for editing.',null,200,$data);}catch(\DomainException$error){$this->result(false,$error->getMessage(),null,423);}catch(\RuntimeException$error){$this->result(false,$error->getMessage(),null,422);}
    }

    public function unlockPageBuilderSection(int$pageId,string$uid):never
    {
        $this->guard();$payload=$this->jsonPayload();$this->builderCsrf($payload);$this->collaborativeBuilderDocument($pageId);$this->result(true,'Section editing lock released.',null,200,$this->cms->releaseBuilderSectionLock($pageId,$uid,$this->auth->id()??0));
    }

    public function comparePageBuilderRevisions(int$pageId):never
    {
        $this->guard();$payload=$this->jsonPayload();$this->builderCsrf($payload);$this->collaborativeBuilderDocument($pageId);$themeSlug=(string)$this->cms->setting('active_theme','sensecms');$catalog=$this->builderCatalog($this->themes->find($themeSlug));$comparison=$this->cms->builderRevisionComparison($pageId,max(0,(int)($payload['from']??0)),max(0,(int)($payload['to']??0)),$catalog);if(!$comparison)$this->result(false,'Choose two available revisions.',null,404);$this->result(true,'Revision comparison prepared.',null,200,$comparison);
    }
    ...
    public function mediaData():never{$this->guard();$scope=$this->access->facilityIds();$this->result(true,'Media library loaded.',null,200,array_merge($this->media->listing($this->mediaFilters(),$scope,max(1,(int)($_GET['page']??1)),24),['folders'=>$this->media->folders($scope),'tags'=>$this->media->tags($scope),'permissions'=>['delete'=>$this->access->allows('content.media.delete'),'global'=>$this->access->allows('content.media.global')]]));}
    ...
    {
        $this->guard();$scope=$this->access->facilityIds();$document=$id?$this->cms->pageAdmin($id):null;if($id&&!$document)$this->result(false,'The page was not found.','/content/pages',404);$languages=$this->cms->languages();$documentSeo=[];
        foreach($languages as$language){$locale=(string)$language['locale'];$stored=$id?(array)$this->cms->setting('seo_page_'.$id.'_'.$locale,[]):[];$translation=(array)($document['translations'][$locale]??[]);$documentSeo[$locale]=array_replace(SeoMeta::documentDefaults('page'),['title'=>(string)($translation['seo_title']??''),'description'=>(string)($translation['seo_description']??'')],$stored);}
        $this->render($id?'Edit page':'New page','page-form',['document'=>$document,'documentSeo'=>$documentSeo,'languages'=>$languages,'pageOptions'=>$this->cms->pages(null,$scope),'pageTemplates'=>$this->pageTemplates(),'facilities'=>$this->facilities->options(false,$scope),'workflowUsers'=>$this->access->workflowUsers($document?(int)$document['facility_id']:null),'workflowHistory'=>$document?$this->workflowRepository->history('page',(int)$document['id']):[]]);
    }
    public function savePage(): never
    {
        $this->guard();if(!$this->auth->verifyCsrf($_POST['csrf']??null))$this->result(false,'Your session token is invalid. Refresh and try again.',null,419);
        try{
            $status=in_array($_POST['status']??'',['draft','published','private','scheduled'],true)?(string)$_POST['status']:'draft';$translations=$this->localizedInput('translations',['title'=>255,'slug'=>255,'excerpt'=>1200,'seo_title'=>255,'seo_description'=>500]);$id=filter_input(INPUT_POST,'id',FILTER_VALIDATE_INT)?:0;$parent=filter_input(INPUT_POST,'parent_id',FILTER_VALIDATE_INT)?:null;$facility=filter_input(INPUT_POST,'facility_id',FILTER_VALIDATE_INT)?:0;if(!$this->facilities->exists($facility))throw new \RuntimeException('Choose a valid facility.');$existing=$id?$this->cms->pageAdmin($id):null;if(array_key_exists('public_path',$_POST)&&\App\Core\PublicPagePath::validate($_POST['public_path'])!==($existing['public_path']??null))$this->access->assert('system.owner');if(in_array($status,['published','private','scheduled'],true))$this->access->assert('content.pages.publish',$facility);if($existing&&in_array((string)$existing['status'],['published','private','scheduled'],true)&&!$this->access->allows('content.pages.publish',(int)$existing['facility_id']))throw new \RuntimeException('Published content can only be edited by a publisher. Duplicate it to create a reviewed revision.');if($existing&&($existing['workflow_state']??'draft')==='in_review'&&!$this->access->allows('content.pages.review',$facility))throw new \RuntimeException('This page is locked while it is awaiting review.');$publishedAt=$this->publicationDate($status,(string)($_POST['published_at']??''));if($parent===$id)$this->result(false,'A page cannot be its own parent.',null,422);$template=trim((string)($_POST['template']??'default'));if(!in_array($template,array_column($this->pageTemplates(),'key'),true))throw new \RuntimeException('Choose a template exposed by the active theme.');
            $languages=$this->cms->languages();$seoInput=is_array($_POST['seo']??null)?$_POST['seo']:[];$pageSeo=[];foreach($languages as$language){$languageCode=(string)$language['locale'];$source=is_array($seoInput[$languageCode]??null)?$seoInput[$languageCode]:[];$stored=$id?(array)$this->cms->setting('seo_page_'.$id.'_'.$languageCode,[]):[];foreach(['og_image','og_image_alt','og_image_width','og_image_height','og_image_type']as$key)if(array_key_exists($key,$source))$stored[$key]=$source[$key];$pageSeo[$languageCode]=SeoMeta::sanitizeDocument($stored,'page');}
            $input=['id'=>$id,'parent_id'=>$parent,'facility_id'=>$facility,'assigned_user_id'=>filter_input(INPUT_POST,'assigned_user_id',FILTER_VALIDATE_INT)?:null,'editorial_note'=>(string)($_POST['editorial_note']??''),'public_path'=>$_POST['public_path']??($existing['public_path']??null),'template'=>$template,'status'=>$status,'visibility'=>($_POST['visibility']??'')==='private'?'private':'public','published_at'=>$publishedAt];$saved=$this->cms->savePage($input,$translations,$this->auth->id()??0);foreach($pageSeo as$languageCode=>$metadata)$this->cms->saveSetting('seo_page_'.$saved.'_'.$languageCode,$metadata);$locale=$this->safeLocale((string)($_POST['ui_locale']??''));$this->result(true,$id?'Page and social image settings updated for every configured language.':'Page created with social image settings for every configured language.','/content/pages/'.$saved.'/edit?lang='.$locale);
        }catch(\InvalidArgumentException $error){$this->result(false,$error->getMessage(),null,422);}catch(\PDOException $error){$this->result(false,$this->duplicateContentMessage($error,'page'),null,422);}catch(\RuntimeException $error){$this->result(false,$error->getMessage(),null,$error->getCode()===403?403:422);}
    }
    ...
    public function postForm(?int $id = null): never { $this->guard();$scope=$this->access->facilityIds();$document=$id?$this->cms->postAdmin($id):null;if($id&&!$document)$this->result(false,'The post was not found.','/content/posts',404);$facility=$document?(int)$document['facility_id']:null;$aiAllowed=$this->access->allows('content.posts.ai',$facility)&&$this->access->allows('content.posts.edit',$facility);$aiProviders=[];if($aiAllowed)foreach($this->ai->enabledProviders('posts')as$provider)$aiProviders[]=['slug'=>(string)$provider['slug'],'name'=>(string)$provider['name'],'driver'=>(string)$provider['driver'],'model'=>(string)$provider['default_model'],'verified'=>(bool)($provider['verified_at']??false)];$this->render($id?'Edit post':'New post','post-form',['document'=>$document,'categories'=>$this->cms->categories(false),'facilities'=>$this->facilities->options(false,$scope),'languages'=>$this->cms->languages(),'workflowUsers'=>$this->access->workflowUsers($facility),'workflowHistory'=>$document?$this->workflowRepository->history('post',(int)$document['id']):[],'postAiAllowed'=>$aiAllowed,'postAiProviders'=>$aiProviders,'postAiActions'=>AiContentService::postActions()]); }
    public function savePost(): never
    {
        $this->guard();if(!$this->auth->verifyCsrf($_POST['csrf']??null))$this->result(false,'Your session token is invalid. Refresh and try again.',null,419);
        try{
            $status=in_array($_POST['status']??'',['draft','published','scheduled'],true)?(string)$_POST['status']:'draft';$translations=$this->localizedInput('translations',['title'=>255,'slug'=>255,'excerpt'=>1200,'content'=>100000,'seo_title'=>255,'seo_description'=>500]);foreach($translations as&$translation)$translation['content']=HtmlSanitizer::sanitize((string)$translation['content']);unset($translation);
            $id=filter_input(INPUT_POST,'id',FILTER_VALIDATE_INT)?:0;$facility=filter_input(INPUT_POST,'facility_id',FILTER_VALIDATE_INT)?:0;if(!$this->facilities->exists($facility))throw new \RuntimeException('Choose a valid facility.');$existing=$id?$this->cms->postAdmin($id):null;if(in_array($status,['published','scheduled'],true))$this->access->assert('content.posts.publish',$facility);if($existing&&in_array((string)$existing['status'],['published','scheduled'],true)&&!$this->access->allows('content.posts.publish',(int)$existing['facility_id']))throw new \RuntimeException('Published content can only be edited by a publisher. Duplicate it to create a reviewed revision.');if($existing&&($existing['workflow_state']??'draft')==='in_review'&&!$this->access->allows('content.posts.review',$facility))throw new \RuntimeException('This post is locked while it is awaiting review.');$publishedAt=$this->publicationDate($status,(string)($_POST['published_at']??''));$tags=is_array($_POST['post_tags']??null)?$_POST['post_tags']:[];
            $input=['id'=>$id,'author_id'=>$this->auth->id(),'facility_id'=>$facility,'assigned_user_id'=>filter_input(INPUT_POST,'assigned_user_id',FILTER_VALIDATE_INT)?:null,'editorial_note'=>(string)($_POST['editorial_note']??''),'category_id'=>filter_input(INPUT_POST,'category_id',FILTER_VALIDATE_INT)?:null,'featured_media_id'=>filter_input(INPUT_POST,'featured_media_id',FILTER_VALIDATE_INT)?:null,'audio_media_id'=>filter_input(INPUT_POST,'audio_media_id',FILTER_VALIDATE_INT)?:null,'video_media_id'=>filter_input(INPUT_POST,'video_media_id',FILTER_VALIDATE_INT)?:null,'ai_knowledge_enabled'=>isset($_POST['ai_knowledge_enabled']),'tags'=>$tags,'status'=>$status,'published_at'=>$publishedAt];$saved=$this->cms->savePost($input,$translations,$this->auth->id()??0);
            foreach($translations as$languageCode=>$translation){$key='seo_post_'.$saved.'_'.$languageCode;$seo=(array)$this->cms->setting($key,[]);$seo['title']=(string)$translation['seo_title'];$seo['description']=(string)$translation['seo_description'];$seo['keywords']=implode(', ',array_slice(array_values(array_unique(array_filter(array_map('trim',preg_split('/[,;\r\n]+/u',(string)($tags[$languageCode]??''))?:[])))),0,30));$this->cms->saveSetting($key,SeoMeta::sanitizeDocument($seo,'post'));}
            $locale=$this->safeLocale((string)($_POST['ui_locale']??''));$this->result(true,$id?'Post updated for every configured language.':'Post created for every configured language.','/content/posts/'.$saved.'/edit?lang='.$locale);
    ...
    public function proposePostAi():never
    {
        $this->guard();$payload=$this->jsonPayload();if(!$this->auth->verifyCsrf($payload['csrf']??null))$this->result(false,'Your session token is invalid. Refresh and try again.',null,419);$facility=max(0,(int)($payload['facility_id']??0));if(!$this->facilities->exists($facility))$this->result(false,'Choose a valid facility before using AI.',null,422);$this->access->assert('content.posts.edit',$facility);$this->access->assert('content.posts.ai',$facility);$post=max(0,(int)($payload['post_id']??0));if($post){$existing=$this->cms->postAdmin($post);if(!$existing||(int)$existing['facility_id']!==$facility)$this->result(false,'The selected post is unavailable in this facility.',null,404);}if(!$this->aiContent)$this->result(false,'The Core AI service is unavailable.',null,503);
        try{$proposal=$this->aiContent->proposePost($this->auth->id()??0,$payload,$this->cms->languages());$this->result(true,'AI post proposal ready for review.',null,200,$proposal);}catch(\RuntimeException$error){$status=$error->getCode()===429?429:($error->getCode()===403?403:422);$this->result(false,$error->getMessage(),null,$status);}catch(\Throwable$error){error_log('Posts AI proposal failed: '.$error->getMessage());$this->result(false,'The AI post proposal could not be completed. Try another configured provider.',null,503);}
    }
    ...
    public function toggleExtension(string $type,string $slug): never
    {
        ...
        try{$this->packages->setActive($type,$slug,$active,$this->auth->id()??0);}catch(\RuntimeException$error){$this->result(false,$error->getMessage(),null,422);}
    ...
    {
        if($this->auth->check() && $this->access->allows('system.manage') && !$this->access->isDemoUser()) $this->updater?->checkIfDue();
    ...
    public function ai(): never { $this->guard(); $this->render('AI & Live Support', 'ai', ['ai' => $this->ai->dashboard(), 'providers' => $this->ai->providers(),'aiDrivers'=>AiProviderClient::drivers()]); }
    public function saveAiProvider():never
    {
        $this->guard();$this->access->assert('ai.manage');if(!$this->auth->verifyCsrf($_POST['csrf']??null))$this->result(false,'Your session token is invalid. Refresh and try again.',null,419);if(!$this->aiContent)$this->result(false,'The Core AI service is unavailable.',null,503);try{$id=$this->aiContent->saveProvider($_POST,$this->auth->id()??0);$this->result(true,'AI provider configuration saved.','/ai?provider='.$id,200,['id'=>$id]);}catch(\RuntimeException$error){$this->result(false,$error->getMessage(),null,422);}
    }
    public function verifyAiProvider(int$id):never
    {
        $this->guard();$this->access->assert('ai.manage');$payload=$this->jsonPayload();if(!$this->auth->verifyCsrf($payload['csrf']??null))$this->result(false,'Your session token is invalid. Refresh and try again.',null,419);if(!$this->aiContent)$this->result(false,'The Core AI service is unavailable.',null,503);try{$this->result(true,'AI provider verified.',null,200,$this->aiContent->verifyProvider($id,$this->auth->id()??0));}catch(\RuntimeException$error){$this->result(false,$error->getMessage(),null,$error->getCode()===429?429:422);}catch(\Throwable$error){error_log('AI provider verification failed: '.$error->getMessage());$this->result(false,'The AI provider could not be verified.',null,503);}
    }
    public function knowledgeBase():never
    {
        $this->guard();$this->access->assert('ai.manage');if(!$this->aiKnowledge)$this->result(false,'The Core Knowledge Base service is unavailable.','/ai',503);$query=mb_substr(trim((string)($_GET['q']??'')),0,120);$type=(string)($_GET['type']??'all');$status=(string)($_GET['status']??'all');$page=max(1,(int)($_GET['page']??1));$edit=max(0,(int)($_GET['edit']??0));$this->render('Knowledge Base','ai-knowledge',['knowledgeSummary'=>$this->aiKnowledge->summary(),'knowledgeDocuments'=>$this->aiKnowledge->documents($query,$type,$status,$page),'knowledgeSiteContent'=>$this->aiKnowledge->siteContent($query,(string)($_GET['content_type']??'all')),'knowledgeTraining'=>$this->aiKnowledge->trainingSummary(),'knowledgeTrainingWorkspace'=>$this->aiKnowledge->trainingWorkspace(),'knowledgeProviders'=>$this->ai->providers(),'knowledgeQuery'=>$query,'knowledgeType'=>$type,'knowledgeStatus'=>$status,'knowledgeEdit'=>$edit?$this->aiKnowledge->document($edit):null,'languages'=>$this->cms->languages()]);
    }
    public function saveKnowledgeDocument():never
    {
        $this->guard();$this->access->assert('ai.manage');if(!$this->auth->verifyCsrf($_POST['csrf']??null))$this->result(false,'Your session token is invalid. Refresh and try again.',null,419);if(!$this->aiKnowledge)$this->result(false,'The Core Knowledge Base service is unavailable.',null,503);try{$kind=($_POST['source_kind']??'manual')==='file'?'file':'manual';$title=(string)($_POST['title']??'');$locale=(string)($_POST['locale']??'');$active=isset($_POST['active']);$id=$kind==='file'?$this->aiKnowledge->saveUpload((array)($_FILES['knowledge_file']??[]),$title,$locale,$active,$this->auth->id()??0):$this->aiKnowledge->saveManual(max(0,(int)($_POST['id']??0)),$title,(string)($_POST['body']??''),$locale,$active,$this->auth->id()??0);$this->result(true,$kind==='file'?'Knowledge file added and indexed.':'Knowledge source saved and indexed.','/ai/knowledge?tab=sources&edit='.$id);}catch(\RuntimeException$error){$this->result(false,$error->getMessage(),null,422);}catch(\Throwable$error){error_log('Knowledge Base save failed: '.$error->getMessage());$this->result(false,'The knowledge source could not be saved.',null,503);}
    }
    public function knowledgeDocumentAction(int$id,string$action):never
    {
        $this->guard();$this->access->assert('ai.manage');if(!$this->auth->verifyCsrf($_POST['csrf']??null))$this->result(false,'Your session token is invalid. Refresh and try again.',null,419);if(!$this->aiKnowledge)$this->result(false,'The Core Knowledge Base service is unavailable.',null,503);try{$this->aiKnowledge->action($id,$action,$this->auth->id()??0);$message=match($action){'delete'=>'Knowledge source and its private file were deleted.','reindex'=>'Knowledge source reindexed.','publish'=>'Knowledge source enabled and indexed.','draft'=>'Knowledge source disabled but preserved.',default=>'Knowledge source archived and removed from retrieval.'};$this->result(true,$message,'/ai/knowledge?tab=sources');}catch(\RuntimeException$error){$this->result(false,$error->getMessage(),null,422);}
    }
    public function saveKnowledgeSource():never
    {
        $this->guard();$this->access->assert('ai.manage');$payload=$this->jsonPayload();if(!$this->auth->verifyCsrf($payload['csrf']??null))$this->result(false,'Your session token is invalid. Refresh and try again.',null,419);if(!$this->aiKnowledge)$this->result(false,'The Core Knowledge Base service is unavailable.',null,503);try{$this->aiKnowledge->setSiteInclusion((string)($payload['type']??''),max(0,(int)($payload['id']??0)),!empty($payload['enabled']),$this->auth->id()??0);$this->result(true,!empty($payload['enabled'])?'Content will be included on the next rebuild.':'Content was excluded from future Knowledge Base rebuilds.',null,200);}catch(\RuntimeException$error){$this->result(false,$error->getMessage(),null,422);}
    }
    public function rebuildKnowledgeBase():never
    {
        $this->guard();$this->access->assert('ai.manage');$payload=$this->jsonPayload();$token=$payload['csrf']??($_POST['csrf']??null);if(!$this->auth->verifyCsrf($token))$this->result(false,'Your session token is invalid. Refresh and try again.',null,419);if(!$this->aiKnowledge)$this->result(false,'The Core Knowledge Base service is unavailable.',null,503);try{$result=$this->aiKnowledge->rebuild($this->auth->id()??0);$message='Knowledge Base rebuilt: '.$result['ready'].' sources ready'.($result['failed']?', '.$result['failed'].' failed':'').'.';$this->result($result['failed']===0,$message,'/ai/knowledge?tab=index',$result['failed']?207:200,$result);}catch(\Throwable$error){error_log('Knowledge Base rebuild failed: '.$error->getMessage());$this->result(false,'The Knowledge Base could not be rebuilt. Review source errors and try again.',null,503);}
    }
    public function saveKnowledgeDataset():never
    {
        $this->guard();$this->access->assert('ai.manage');if(!$this->auth->verifyCsrf($_POST['csrf']??null))$this->result(false,'Your session token is invalid. Refresh and try again.',null,419);if(!$this->aiKnowledge)$this->result(false,'The Core Knowledge Base service is unavailable.',null,503);try{$id=$this->aiKnowledge->saveDataset(max(0,(int)($_POST['id']??0)),(string)($_POST['name']??''),(string)($_POST['description']??''),filter_input(INPUT_POST,'provider_id',FILTER_VALIDATE_INT)?:null,(string)($_POST['base_model']??''),$this->auth->id()??0);$this->result(true,'Training dataset saved.','/ai/knowledge?tab=training&dataset='.$id);}catch(\RuntimeException$error){$this->result(false,$error->getMessage(),null,422);}
    }
    public function saveKnowledgeExample():never
    {
        $this->guard();$this->access->assert('ai.manage');if(!$this->auth->verifyCsrf($_POST['csrf']??null))$this->result(false,'Your session token is invalid. Refresh and try again.',null,419);if(!$this->aiKnowledge)$this->result(false,'The Core Knowledge Base service is unavailable.',null,503);try{$this->aiKnowledge->saveTrainingExample(max(0,(int)($_POST['dataset_id']??0)),(string)($_POST['input_text']??''),(string)($_POST['ideal_output']??''),(string)($_POST['instructions']??''),(string)($_POST['locale']??''),isset($_POST['approved']),$this->auth->id()??0);$this->result(true,'Training example added for review.','/ai/knowledge?tab=training');}catch(\RuntimeException$error){$this->result(false,$error->getMessage(),null,422);}
    }
    public function saveKnowledgeEvaluation():never
    {
        $this->guard();$this->access->assert('ai.manage');if(!$this->auth->verifyCsrf($_POST['csrf']??null))$this->result(false,'Your session token is invalid. Refresh and try again.',null,419);if(!$this->aiKnowledge)$this->result(false,'The Core Knowledge Base service is unavailable.',null,503);try{$this->aiKnowledge->saveEvaluation((string)($_POST['name']??''),(string)($_POST['question']??''),(string)($_POST['expected_answer']??''),(string)($_POST['locale']??''),(string)($_POST['required_source_key']??''),$this->auth->id()??0);$this->result(true,'Evaluation case saved.','/ai/knowledge?tab=training');}catch(\RuntimeException$error){$this->result(false,$error->getMessage(),null,422);}
    }
    public function knowledgeTrainingAction(string$type,int$id,string$action):never
    {
    ...
    {
        $file=$_FILES[$field]??null; if(!is_array($file)||($file['error']??UPLOAD_ERR_NO_FILE)!==UPLOAD_ERR_OK||!is_uploaded_file($file['tmp_name'])||(int)filesize($file['tmp_name'])>MediaLibrary::MAX_VIDEO) throw new \RuntimeException('Upload a valid MP4 video up to 80 MiB.');
    ...
    private function builderCsrf(array$payload):void{if(!$this->auth->verifyCsrf($payload['csrf']??null))$this->result(false,'Your session token is invalid. Refresh and try again.',null,419);}
    private function collaborativeBuilderDocument(int$pageId):array{$document=$this->cms->pageAdmin($pageId);if(!$document)$this->result(false,'The selected page does not exist.',null,404);$this->access->assert('content.pages.collaborate',(int)$document['facility_id']);return$document;}
    private function editableBuilderDocument(int$pageId):array{$document=$this->cms->pageAdmin($pageId);if(!$document)$this->result(false,'The selected page does not exist.',null,404);$this->access->assert('content.pages.edit',(int)$document['facility_id']);if(in_array((string)$document['status'],['published','scheduled','private'],true)&&!$this->access->allows('content.pages.publish',(int)$document['facility_id']))$this->result(false,'Published content can only be edited by a publisher. Duplicate it to create a reviewed revision.',null,403);if(($document['workflow_state']??'draft')==='in_review'&&!$this->access->allows('content.pages.review',(int)$document['facility_id']))$this->result(false,'This page is locked while it is awaiting review.',null,423);return$document;}
    ...
    {
        return ['enabled' => false, 'width' => 1040, 'image_width' => 390, 'height' => 645, 'eyebrow' => '', 'title' => '', 'text' => '', 'cta_label' => '', 'cta_url' => '', 'image_url' => '', 'start_at' => '', 'end_at' => '', 'frequency' => 'session', 'background_color' => '#123b4a', 'accent_color' => '#e96c4a', 'text_color' => '#ffffff', 'icon_color' => '#f4c95d', 'facts' => [], 'localized' => []];
    }
