...
return static function(string$method,string$path,ExtensionContext$context):bool{
    ...
    if($method==='GET'&&$path==='/social-publishing/telegram-channels'){$provider=null;foreach($manager->catalog()as$item)if($item['slug']==='telegram-channels-publisher'){$provider=$item;break;}$context->dashboard->extensionPage('Telegram Channels Publisher',__DIR__.'/views/telegram-channels.php',['telegramChannelsProvider'=>$provider,'csrf'=>$context->auth->csrf(),'extensionActive'=>'/social-publishing','extensionStyles'=>['/extension-assets/addon/social-publishing/social.css?v=0.2.2','/extension-assets/plugin/telegram-channels-publisher/telegram-channels.css?v=0.1.1']]);}
