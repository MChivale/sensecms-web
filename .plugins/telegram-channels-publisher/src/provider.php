...
return new class {
    ...
    private function client():TelegramChannelsClient{if($this->root===''||!is_dir($this->root))throw new RuntimeException('The Telegram Channels provider runtime is unavailable.');$runtime=new Runtime($this->root);$installed=$runtime->read('installed');$root=$this->root;$baseUrl=$runtime->baseUrl();$config=require$root.'/config/workspace.php';return new TelegramChannelsClient($runtime->license(),(string)($config['integrations']['telegram_broker_url']??''),(string)($config['base_url']??''));}
};
