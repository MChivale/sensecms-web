...
return static function(string$method,string$path,ExtensionContext$context):bool{
    ...
    if($method==='GET'&&$path==='/social-publishing/pinterest'){
        ...
        $context->dashboard->extensionPage('Pinterest Publisher',__DIR__.'/views/pinterest.php',['pinterestProvider'=>$provider,'pinterestOAuthRevision'=>(int)($_SESSION['pinterest_oauth_revision']??0),'pinterestOAuthOutcome'=>(string)($_SESSION['flash_type']??''),'csrf'=>$context->auth->csrf(),'extensionActive'=>'/social-publishing','extensionStyles'=>['/extension-assets/addon/social-publishing/social.css?v=0.2.2','/extension-assets/plugin/pinterest-publisher/pinterest.css?v=0.1.1'],'extensionScripts'=>['/extension-assets/plugin/pinterest-publisher/pinterest.js?v=0.1.1']]);
    }
